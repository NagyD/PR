...
¯¯¯¯¯¯¯¯¯¯
 ...
 This file was created from rle_compress.c by David.
  Version: (2011-Feb-10)
...
void compressRleC(const unsigned char* input, int inputSize,
	...
	while (cursorPix<imgEnd) {
		...
		counter=(unsigned char*)(cursorData++);
		*counter=255;
		...
		while (
			...
			((*counter)!=127) && *cursorPix!=0 /* Important: zeroes must be encoded as a repeated string */
				/* otherwise, the pixels with color zero won't be transparent. */
		) {
		...
		if (*counter==255) {
			*counter=0x80;
			while ((cursorPix+1<imgEnd)&&(*cursorPix==(*(cursorPix+1)))&&((*counter)!=/*-128*/255)) {
				cursorPix++;
				(*counter)++;
			}
